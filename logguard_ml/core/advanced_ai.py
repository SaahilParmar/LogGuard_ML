...
try:
    ...
    logger.warning(
        f"Transformers not available: {e}. Install with: pip install transformers"
    )
...
try:
    ...
    logger.warning(
        f"AutoML dependencies not available: {e}. Install with: pip install optuna scikit-learn"
    )
...
try:
    ...
    logger.warning(
        f"Explainability libraries not available: {e}. Install with: pip install shap lime"
    )
...
class TransformerLogAnalyzer:
    ...
    def encode_log_messages(
        self, messages: List[str], max_length: int = 512
    ) -> np.ndarray:
        """
...
class AdaptiveLearningDetector:
    ...
    def _retrain_with_feedback(self):
        ...
        y_feedback = np.array(
            [1 if item["correct"] else -1 for item in self.feedback_buffer]
        )
...
class MultiModalAnomalyDetector:
    ...
    def _extract_temporal_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        for ts in timestamps:
            features = [ts.hour, ts.minute, ts.day, ts.weekday(), ts.month]
...
class ExplainableAnomalyDetector:
    ...
    def explain_prediction(self, log_message: str, features: np.ndarray) -> Dict:
        ...
        confidence = getattr(self.base_detector, "decision_function", lambda x: [0])(
            [features]
        )[0]
...
class RealTimeMLPipeline:
    ...
    def process_stream(self, log_entry: Dict) -> Dict:
        ...
        if self.model:
            ...
            confidence = getattr(self.model, "decision_function", lambda x: [0])(
                [features]
            )[0]
        else:
            prediction = 0
