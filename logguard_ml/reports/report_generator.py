...
def _create_summary_stats(df: pd.DataFrame) -> Dict:
    ...
    if "is_anomaly" in df.columns:
        ...
        stats["anomaly_percentage"] = (stats["anomaly_count"] / len(df) * 100) if len(df) > 0 else 0
    
    # Log level statistics
    if "level" in df.columns:
        level_counts = df["level"].value_counts()
        stats["error_count"] = int(level_counts.get("ERROR", 0))
        stats["warn_count"] = int(level_counts.get("WARN", 0) + level_counts.get("WARNING", 0))
...
def _create_visualizations(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        if "level" in df.columns:
            ...
            plots_html.append(level_fig.to_html(div_id="level-chart", include_plotlyjs=False))
        
        # 2. Anomaly Timeline
        if "timestamp" in df.columns and "is_anomaly" in df.columns:
            timeline_fig = _create_anomaly_timeline(df)
            if timeline_fig:
                plots_html.append(timeline_fig.to_html(div_id="timeline-chart", include_plotlyjs=False))
        
        # 3. Message Length Distribution
        if "message" in df.columns:
            length_fig = _create_message_length_distribution(df)
            plots_html.append(length_fig.to_html(div_id="length-chart", include_plotlyjs=False))
...
def _create_anomaly_table(df: pd.DataFrame) -> str:
    ...
    table_html = anomalies[display_columns].head(50).to_html(
